...
public class SoundFx {
    ...
    public static final int COLLECT_0 = 5; // ..COLLECT_0 + 7, pitch rises with combo
    ...
    public SoundFx() {
        try {
            tracks[TICK] = make(square(1100f, 0.045f, 0.18f));
            ...
            // pentatonic-ish ladder so chained pickups literally sound like climbing
            float[] steps = {523f, 587f, 659f, 784f, 880f, 1047f, 1175f, 1319f};
            for (int i = 0; i < 8; i++) {
                tracks[COLLECT_0 + i] = make(sine(steps[i], 0.10f, 0.4f));
            }
    ...
    private static short[] square(float freq, float dur, float vol) {
        int n = (int) (RATE * dur);
        short[] out = new short[n];
        for (int i = 0; i < n; i++) {
            double s = Math.sin(2 * Math.PI * freq * i / RATE) > 0 ? 1 : -1;
            double env = Math.exp(-5.0 * i / n);
            out[i] = (short) (s * env * vol * 32767);
        }
        return out;
    }
